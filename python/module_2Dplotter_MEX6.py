...
class Plot2D:
    ...
    def Plot(self):
        ...
        self.ax1.set_title("2D distribution MEX-6")
    ...
    def conc_calcCpp(self, X_list, Y_list, Z_list, id_list):
        ...
        # normalisation of the plot colors intensities with vmin and vmax on the total MEX-6
        v_min = np.min(hist2)
        v_max = np.max(hist2)
        ...
        ax_id_conc.plot(xbins_line, id0, "r", label="MEX-6s")
        ax_id_conc.plot(xbins_line, id1, "b", label="MEX-6f")
        ax_id_conc.set(
        ...
        av_velocity.set(
            ...
            ylabel="Mean velocity MEX-6s, MEX-6f",
            xlabel="Embryo length (um)",
        )
        ...
        # if self.counter % autosave == 0:
        #  plt.imsave(os.path.join(self.path, f'Graphs/MEX-6-PlotB_t-{self.counter}.png'), np.array(fig_root_based.canvas.renderer.buffer_rgba()))
        # plt.show(block=False)
        # plt.pause(0.1)
        del histo3DSlow
        del histo3DFast
