...
class Plot2D:
    ...
    def plot(self):
        ...
        for xbin in range(point1[0], point2[0]):
            oneD.append(
                hist[xbin][point2[1]]
            )  # original statement int(point2[1] / density)
    ...
    def conc_calcCpp(self, X_list, Y_list, Z_list, id_list):
        autosave = 100  # setting frequency of image saving
        self.counter += 1
        ...
        hist3, xbins3, ybins3, im3 = ax_id3.hist2d(
            x4, y4, bins=self.limits[0][1], range=self.limits
        )
        ax_id3.set(
            title="PLK-1",
            xlabel="Long axis (um)",
        ...
        if self.counter % autosave == 0:
            plt.imsave(
                os.path.join(self.path, f"Graphs/PLK-1-Embryo_t-{self.counter}.png"),
        ...
        # root stuff
        self.histo3DSlow = ROOT.TH3F(
            "plot3D-Slow", "plot3D-Slow", 50, 0, 50, 30, 0, 30, 30, 0, 30
        )
        self.histo3DFast = ROOT.TH3F(
            "plot3D-Fast", "plot3D-Fast", 50, 0, 50, 30, 0, 30, 30, 0, 30
        )
        self.histo3DUnbound = ROOT.TH3F(
            "histo3DUnbound", "histo3DUnbound", 50, 0, 50, 30, 0, 30, 30, 0, 30
        )
        ...
        max_arr = (
            np.array(slow_content) + np.array(fast_content) + np.array(unbound_content)
        )
        self.data_movie_2 = max_arr / np.max(max_arr)

        v_average = (
            np.array(slow_content) * self.v_mex5_slow
            + np.array(fast_content) * self.v_mex5_fast
            + np.array(unbound_content) * self.v_plk1
        ) / (
            np.array(slow_content) + np.array(fast_content) + np.array(unbound_content)
        )
        ...
        self.ax_id_conc_root.plot(
            xbins_line, conc_root_mex5_slow, "r", label="PLK-1 to MEX-5s"
        )
        self.ax_id_conc_root.plot(
            xbins_line, conc_root_mex5_fast, "b", label="PLK-1 to MEX-5f"
        )
        self.ax_id_conc_root.plot(
            xbins_line, conc_root_plk1_unbound, "g", label="Unbound PLK-1"
        )
    ...
    def DrawMovie(self):
        ...
        im_ani.save(
            os.path.join(self.path, "2DPlk1.html"), writer="imagemagick", fps=10, dpi=50
        )
        ...
        im_ani2.save(
            os.path.join(self.path, "GradientPlk1.html"),
            writer="imagemagick",
            fps=10,
            dpi=50,
        )
